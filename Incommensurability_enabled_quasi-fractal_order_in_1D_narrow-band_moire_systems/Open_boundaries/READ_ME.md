...
- `<filename>`
  - Data: Each row contains $\phi$ value, maxdim, deltaEnergy, eVariance, deltaS, and NsweepsTot for each $\phi$ configuration, where:
    ...
    - eVariance: Energy variance for the last sweep, defined as $\Delta E_var \langle H^2 \rangle - \langle H \rangle^2$.
...
- `S.<filename>`
  - Data: $\phi$ value and corresponding single-particle entanglement entropy defined as $-\Tr[C\log C]=-\sum_{\alpha=1}^{N}n_{\alpha}\log(n_{\alpha})$ for each $\phi$ configuration.

- `IPR.<filename>`
  - Data: $\phi$ value and corresponding IPR of natural orbitals, defined as ${\rm IPR}_{{\rm sp}}=\sum_{\alpha=1}^{N}n_{\alpha}\sum_{j=1}^{N}|\phi_{\alpha}(j)|^{4}$, for each $\phi$ configuration.
