...
int main(int argc,char **argv)
{
	...
	int maximumDim=10000; //Very large maxdim --> I will let DMRG use the largest one needed for the chosen cutoff
	...
	// If a given parameter is sent as "VARY", the code assumes that this parameter will be varied
	...
	auto id=atoi(argv[1]); //Calculation identifier --> useful to distinguish between calculations with different internal parameters
	...
	auto sitecut = int(N/2.0); // to compute entanglement entropy 
	...
	unsigned int exclusionparams = exclusion-2; //We have id,maxcutoffExp and only then params (SEE BELOW!)
	...
	//Below we define the configuration for the remaining sweeps that we need to take until the convergence criteria are satisfied
	...
	for(auto par=parinit;par<=parfinal+deltapar/2.0;par+=deltapar){
		...
		auto fnameExcludepar = filenameExcludeParams(VarNames, VarValsString,VarExclusions);
		
		// DMRG calculation
		auto [energy,psi,deltaEnergy,eVariance,deltaSEnt,NsweepsTot] = DMRGSweepsFullNoInitState( sitesElec, params, sweeps, RemainingSweeps, deltaEnergyCrit,  eVarianceCrit, deltaSEntCrit, NsweepsMax, fname);				
		...
		/* ----- Compute correlation matrix C_ij=<c^\dagger_i c_j> and its eigenvalues and eigenvectors (natural orbitals) ------ */
		...
		for(int row=1;row<=N;row++){
			for(int col=1;col<=N;col++){
				...
				if(row==col)
					niVec.push_back(val.real());//<ni> is real anyway!		
			}
		}
		...
		for(auto iter=1;iter<=N;iter++) 
			occupanciesVec.push_back(elt(Dmat,DmatInd=iter,prime(DmatInd)=iter));		
		...
		/* Compute IPR of natural orbitals as defined in https://journals.aps.org/prl/pdf/10.1103/PhysRevLett.115.046603 (eq.5) */
		...
		for(auto evecindex=1;evecindex<=N;evecindex++){
			...
			for(auto row=1;row<=N;row++){
				auto eveci=fabs(eltC(Umat,UmatRowIndex=row,UmatColIndex=evecindex));
				ipr+=eveci*eveci*eveci*eveci;
				norm+=eveci*eveci;
			}
			iprfull+=na*ipr;
			if(fabs(norm-1)>1.E-8)
				std::cout << "Problem! Eigenvectors of one-particle density matrix not normalized! Norm: "  << norm << std::endl;
		}
		...
		for(int k=0;k<=Ndivsq;k++)
			qlist.push_back(2*M_PI/Ndivsq*k); 
		...
		MPI_Gather(&Sent, 1, MPI_DOUBLE, SentlistWorld.data(), 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
		MPI_Gather(occupanciesVec.data(), N, MPI_DOUBLE, occupanciesVecWorld.data(), N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
		MPI_Gather(CMatVec.data(), 2*N*N, MPI_DOUBLE, CMatVecWorld.data(), 2*N*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);// the *2 is because CMatVec and CMatVecWorld are vectors of complex numbers	
		MPI_Gather(niVec.data(), N, MPI_DOUBLE, nivecWorld.data(), N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
		MPI_Gather(structFactorList.data(), 2*Npointsq, MPI_DOUBLE, structFactorListWorld.data(), 2*Npointsq, MPI_DOUBLE, 0, MPI_COMM_WORLD);		
		MPI_Gather(&SsinglePart, 1, MPI_DOUBLE, SsinglePartWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
		MPI_Gather(&iprfullOverNp, 1, MPI_DOUBLE, iprfullOverNpWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
		MPI_Gather(&F, 1, MPI_DOUBLE, FWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
		MPI_Gather(&SqPi, 2, MPI_DOUBLE, SqPiWorld, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD);// the 2 is because SqPi is a complex number
				
		MPI_Gather(&energy, 1, MPI_DOUBLE, energyWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
		MPI_Gather(&deltaEnergy, 1, MPI_DOUBLE, deltaEnergyWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
		MPI_Gather(&eVariance, 1, MPI_DOUBLE, eVarianceWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
		MPI_Gather(&deltaSEnt, 1, MPI_DOUBLE, deltaSEntWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);	
		MPI_Gather(&NsweepsTot, 1, MPI_INT, NsweepsTotWorld, 1, MPI_INT, 0, MPI_COMM_WORLD);			
		...
		//write to files only from root process after gathering data, to avoid problems
		if (my_rank == 0){	
		
			std::ofstream outfile;
			outfile.open("Data_Energy/"+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++)
				outfile << phiWorld[ind] << " " << std::setprecision(dataPrecision)  << energyWorld[ind] << "\n"; 	
			outfile.close();
			
			//Save convergence info
			outfile.open("Data_Convergence/"+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++)
				outfile << phiWorld[ind] << " " << maxdimWorld[ind]  << " " <<  std::setprecision(dataPrecision) << deltaEnergyWorld[ind] << " "  <<  std::setprecision(dataPrecision) << eVarianceWorld[ind] << " " <<  std::setprecision(dataPrecision) << deltaSEntWorld[ind] << " " <<  std::setprecision(dataPrecision) << NsweepsTotWorld[ind] << "\n"; 
			outfile.close();	
					
			//Save entanglement entropy
			outfile.open("Data_EE/"+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++)
			...
			for(int ind1=0;ind1<world_size;ind1++){
				...
				for(int ind2=0;ind2<N;ind2++){
					outfile << std::setprecision(dataPrecision)  << occupanciesVecWorld[ind1*N+ind2] << " ";
				}
				outfile << "\n";
			} 
			outfile.close();
			...
			for(int ind1=0;ind1<world_size;ind1++){
				outfile << phiWorld[ind1] << " ";
				for(int ind2=0;ind2<N;ind2++){
					outfile << std::setprecision(dataPrecision)  << nivecWorld[ind1*N+ind2] << " ";
				}
				outfile << "\n";
			} 
			outfile.close();					
			...
			outfile.open("Data_C/S."+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++)
				outfile << phiWorld[ind] <<" " << std::setprecision(dataPrecision)  << SsinglePartWorld[ind] << "\n"; 	
			outfile.close();	
			
			//Save single particle IPR
			outfile.open("Data_C/IPR."+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++)
				outfile << phiWorld[ind] <<" " << std::setprecision(dataPrecision)  << iprfullOverNpWorld[ind] << "\n"; 	
			outfile.close();
			
			//Save Fidelity
			if(par>parinit){
				outfile.open("Data_F/"+fnameExcludepar,std::ios_base::app); // append instead of overwrite
				for(int ind=0;ind<world_size;ind++)
					outfile << par-deltapar/2.0 << " " << phiWorld[ind] << " " << std::setprecision(dataPrecision)  << FWorld[ind] << "\n"; 	
				outfile.close();
			}
			
			
			//Save all structure factors
			outfile.open("Data_St/"+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++){
				if(ind==0){
			...
			outfile.open("Data_St/SqPi."+fnameExcludepar,std::ios_base::app); // append instead of overwrite
			for(int ind=0;ind<world_size;ind++){						 
				outfile << phiWorld[ind] << " "  << std::setprecision(dataPrecision)  << SqPiWorld[ind] << "\n";	
			}														
			outfile.close();	
