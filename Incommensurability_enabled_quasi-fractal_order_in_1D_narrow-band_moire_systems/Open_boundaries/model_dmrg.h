...
This file contains functions to define the model
...
auto model(auto sitesElec, auto params){
	...
	for(int i = 1; i < N; ++i){
		ampo += U,"N",i,"N",i+1;
		ampo += -t*(1.0+V2*cos( 2*M_PI*tau*(i-1+1/2.0) + phi) ),"Cdag",i,"C",i+1;
		ampo += -t*(1.0+V2*cos( 2*M_PI*tau*(i-1+1/2.0) + phi) ),"Cdag",i+1,"C",i;			
	}
	for(int i = 1; i <= N; ++i){  
