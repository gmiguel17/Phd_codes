...
int main(int argc, char **argv) {
  ...
  auto deltaEnergyCrit = 1E-6;
  auto eVarianceCrit = 5 * 1E-6;
  auto deltaSEntCrit = 1E-3;
  auto NsweepsMax = 500;
  ...
  auto Nuc = N;  // Number of unit cells

  // For structure factor
  auto Ncphi = int(sqrt(world_size));
  auto phiinit = 3.14159265359 / 4.0;
  auto deltaphi = 2.0 * M_PI / Ncphi / Nuc;
  auto Nck = int(sqrt(world_size));
  auto kinit = 0.0;
  auto deltak = 2.0 * M_PI / Nck;
  std::vector<std::tuple<double, double> > phikConfs;
  for (int iphi = 0; iphi < Ncphi; iphi++) {
    for (int ik = 0; ik < Nck; ik++) {
      auto phiin = phiinit + iphi * deltaphi;
      auto kin = kinit + ik * deltak;
      phikConfs.push_back(std::make_tuple(phiin, kin));
    }
  }
  auto NcTotal = Ncphi * Nck;
  ...
  for (auto par = parinit; par <= parfinal + deltapar / 2.0; par += deltapar) {
    ...
    auto phik = phikConfs[my_rank];
    auto phi = std::get<0>(phik);
    auto k = std::get<1>(phik);
    ...
    unsigned int phiindexVars = 9;
    unsigned int kindexVars = 10;

    println("\n **** Variable parameter: ", params[exclusionparams],
            " **** \n");

    // Definitions for file names
    std::vector<string> VarNames = {"id", "maxCutoff", "N",  "Np",  "N2", "t",
                                    "U",  "V",         "V2", "phi", "k"};
    ...
    std::vector<unsigned int> VarExclusions = {
        phiindexVars,
        kindexVars};  // Exclude positions from 0 to VarNames.size()
    ...
    // COMPUTE QUANTITIES USING the MPS psi READ FROM FILE
    // ///////////////////////////////////////////////////////////////
    ...
    auto DmatInd =
        commonIndex(Umat, Dmat);  // Dmat has indices DmatInd and prime(DmatInd)
    Print(norm(CMat - dag(Umat) * Dmat * prime(Umat)));
    ...
    for (auto evecindex = 1; evecindex <= N; evecindex++) {
      ...
      if (fabs(norm - 1) > 1.E-8)
        std::cout << "Problem! Eigenvectors of OPDM not normalized! Norm: "
                  << norm << std::endl;
    }
    ...
    if (my_rank == 0) {
      ...
      kWorld = (double *)malloc(sizeof(double) * world_size);
    ...
    MPI_Gather(&k, 1, MPI_DOUBLE, kWorld, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    if (my_rank == 0) {
      ...
      for (int ind = 0; ind < world_size; ind++)
        outfile << phiWorld[ind] << " " << kWorld[ind] << " "
                << std::setprecision(dataPrecision) << energyWorld[ind] << "\n";
      outfile.close();
      ...
      for (int ind = 0; ind < world_size; ind++)
        outfile << phiWorld[ind] << " " << kWorld[ind] << " "
                << maxdimWorld[ind] << " " << std::setprecision(dataPrecision)
                << deltaEnergyWorld[ind] << " "
                << std::setprecision(dataPrecision) << eVarianceWorld[ind]
                << " " << std::setprecision(dataPrecision)
                << deltaSEntWorld[ind] << " "
                << std::setprecision(dataPrecision) << NsweepsTotWorld[ind]
                << "\n";
      outfile.close();
      ...
      for (int ind1 = 0; ind1 < world_size; ind1++) {
        ...
        auto kaux = std::get<1>(phik);
        auto auxVarValsString = VarValsString;
        auxVarValsString[phiindexVars] = std::to_string(phiaux);
        auxVarValsString[kindexVars] = std::to_string(kaux);

        auto fnamevarPhik = filename(VarNames, auxVarValsString);
        outfile.open("Data_C/CMat." +
                     fnamevarPhik);  // I have to use one name per process
      ...
      for (int ind1 = 0; ind1 < world_size; ind1++) {
        outfile << phiWorld[ind1] << " " << kWorld[ind1] << " ";
      ...
      for (int ind = 0; ind < world_size; ind++)
        outfile << phiWorld[ind] << " " << kWorld[ind] << " "
                << std::setprecision(dataPrecision) << SsinglePartWorld[ind]
                << "\n";
      outfile.close();
      ...
      for (int ind = 0; ind < world_size; ind++)
        outfile << phiWorld[ind] << " " << kWorld[ind] << " "
                << std::setprecision(dataPrecision) << iprfullOverNpWorld[ind]
                << "\n";
      outfile.close();
      ...
      if (par > parinit) {
        ...
        for (int ind = 0; ind < world_size; ind++)
          outfile << par - deltapar / 2.0 << " " << phiWorld[ind] << " "
                  << kWorld[ind] << " " << std::setprecision(dataPrecision)
                  << FWorld[ind] << "\n";
        outfile.close();
      }
      ...
      for (int ind = 0; ind < world_size; ind++) {
        outfile << phiWorld[ind] << " " << kWorld[ind] << " "
                << std::setprecision(dataPrecision) << SqPiWorld[ind] << "\n";
      }
      outfile.close();
