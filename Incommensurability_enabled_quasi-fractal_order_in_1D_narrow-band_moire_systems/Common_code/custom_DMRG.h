...
auto dmrgFromModel(auto sitesElec, auto params , auto sweeps){
	...
	auto nocc=0;
	for(auto i=1;i<=N;i++){//i should go from 1 to N (otherwise gives out of range error)
		if(i%2==0 && nocc<Np) {
		state.set(i,"Occ"); 
		printf("%d ",i);
		nocc++;
...
// Keep sweeping as specified by "RemainingSweeps" untill convergence specified by criteria deltaEnergyCrit,deltaSEntCrit and eVarianceCrit is reached
// If the number of sweeps reaches NsweepsMax, the calculation also stops
auto SweepsToConvergence(auto &sitesElec, auto params, auto &RemainingSweeps, auto energy, auto Sent, auto &psi, auto deltaEnergyCrit, auto eVarianceCrit, auto deltaSEntCrit, auto NsweepsMax, auto fname){
	...
	auto energynew=0.0;
	auto Sentnew = 0.0;		
	while((deltaEnergy>deltaEnergyCrit || deltaSEnt>deltaSEntCrit || eVariance>eVarianceCrit) && swstep<=NsweepsMax ){
	...
	return std::make_tuple(energy,psi,deltaEnergy,eVariance,deltaSEnt,swstep);

}

// DMRG calculation starting with random initial MPS (see model_dmrg.cc)
auto DMRGSweepsFullNoInitState(auto &sitesElec, auto params, auto &sweeps, auto &RemainingSweeps, auto deltaEnergyCrit, auto eVarianceCrit, auto deltaSEntCrit, auto NsweepsMax, auto fname){
	...
	auto final_result=SweepsToConvergence(sitesElec, params, RemainingSweeps, energy, Sent, psi, deltaEnergyCrit, eVarianceCrit, deltaSEntCrit, NsweepsMax,fname);
	...
	return std::make_tuple(energy,psi,deltaEnergy,eVariance,deltaSEnt,NsweepsTot);	
}

// This DMRG calculation uses psiold as the initial state for the first sweep **IF** loopindex>0
// Useful for fidelity calculations, where the MPS in for a set of parameters can be used as initial 
// state for the next set of parameters
auto DMRGSweepsFull(auto loopindex, auto &sitesElec, auto params, auto &sweeps, auto &RemainingSweeps, auto psiold, auto deltaEnergyCrit, auto eVarianceCrit, auto deltaSEntCrit, auto NsweepsMax, auto fname){
