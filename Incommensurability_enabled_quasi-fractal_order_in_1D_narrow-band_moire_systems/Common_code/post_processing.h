...
// Compute entanglement entropy cutting the system between site b and b+1 (subsystem A: 1 to b; subsytem B: b+1 to N)
...
auto Cij(auto sitesElec, auto &psi, auto i, auto j){
	...
	auto Adag_i = op(sitesElec,"Adag",i);//automatically returns tensor on site 
...
/* ------------------ FUNCTIONS TO COMPUTE KOHN'S LOCALIZATION TENSOR - AutoMPO Way (just to cross-check) -------------------------- */

auto R_MPO(auto sitesElec, auto &psi, auto N){

	auto ampo = AutoMPO(sitesElec); 
	for(auto i=1;i<=N;i++)
		ampo+=(i-1)*1.0,"Cdag",i,"C",i; 
	auto mpo = toMPO(ampo); 
	auto result = innerC(psi,mpo,psi);
...
/* ------------------ FUNCTIONS TO COMPUTE KOHN'S LOCALIZATION TENSOR - efficient -------------------------- */
...
//AutoMPS way
auto psiNp_cd_psiNpm1_AUTOMPS(auto sitesElec, auto N, auto &psiNp, auto &psiNpm1){
...
// Efficient way
auto psiNp_cid_psiNpm1(auto sitesElec, auto N, auto &psiNp, auto &psiNpm1, auto i){
